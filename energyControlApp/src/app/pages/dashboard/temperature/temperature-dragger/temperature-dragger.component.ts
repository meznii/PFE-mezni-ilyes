...
export class TemperatureDraggerComponent implements AfterViewInit, OnChanges , OnInit {
  ...
  ngOnInit(): void {
    if (this.deviceStatus === 'no_alarm') {
      this.colorStatus = this.colorDevice[0].success;
    }
    if ((this.deviceStatus === 'low_alarm') || (this.deviceStatus === 'low_warning')) {
      this.colorStatus = this.colorDevice[0].warning;
    }
    if ((this.deviceStatus === 'high_warning' ) || (this.deviceStatus === 'high_alarm') ) {
      this.colorStatus = this.colorDevice[0].danger;
    }
  }
  ...
  switchPower() {
    ...
    // this.apisService.putModifierEtatdevice({'data': 'data'}, 1).subscribe(
    //   (response) => {
    //     console.log('response ', response);
    //     console.log('power function is here');
    //   },
    // );
